...
require_once($_SERVER['DOCUMENT_ROOT'].'/nexuscare/tools/mypathdb.php');
...
if ($option == "incluir") {
    ...
    // Denegar creación de citas desde cuentas de paciente (UI restringe, servidor también)
...
if ($option == "modificar") {
    ...
    // No permitir que el paciente modifique citas (UI restringe, servidor refuerza)
    if ($rolSesion == "paciente") {
        echo json_encode([
            "error" => 8,
            "mensaje" => "No tiene permisos para modificar citas."
        ]);
        exit;
    }
...
if ($option == "eliminar") {
    ...
    // Denegar que pacientes cancelen citas desde la API
    if ($rolSesion == "paciente") {
        echo json_encode([
            "error" => 1,
            "mensaje" => "No tiene permisos para cancelar citas."
        ]);
        exit;
    }

    if ($rolSesion == "medico" || $rolSesion == "médico") {
        // El médico puede cancelar sólo sus propias citas
        if (intval($cita["id_medico"]) != $idSesion) {
            echo json_encode([
                "error" => 1,
