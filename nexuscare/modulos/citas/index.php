...
<script src="/nexuscare/modulos/citas/citasFunciones.js"></script>
<script src="/nexuscare/modulos/pacientes/pacientesFunciones.js"></script>

<script>
    // Delegated handler: llama a VerPaciente pasando el data-paciente
    (function () {
        $(document).on('click', '.btn-ver-paciente', function () {
            var id = $(this).data('paciente');
            if (!id) return;
            if (typeof VerPaciente === 'function') {
                VerPaciente(id);
            } else {
                console.error('VerPaciente no está definida');
            }
        });
    })();
</script>
